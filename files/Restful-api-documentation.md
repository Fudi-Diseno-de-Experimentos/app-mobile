...
- `POST /api/v1/events` - Create a new event.
  - **Input Example**: `{ "title": "Q1 Planning", "description": "Sales sync", "date": "2025-02-15T14:30:00", "location": "Room A", "recipientIds": ["uuid1", "uuid2"], "createdBy": "uuid" }`
...
- `POST /api/v1/companies` - Create a new company profile.
  - **Input Example**: `{ "ruc": "12345678901", "nombre": "Tech Corp", "iconUrl": "url", "isActive": true, "userId": "uuid" }`
...
- `POST /api/v1/users/me/company/join` - Allows an authenticated user to join a company using a 6-character join code.
  - **Description**: The authenticated user provides a valid join code. If the code matches an existing company, the user is assigned to that company. The endpoint requires authentication (JWT or session).
  - **Input Example**:
    ```json
    { "joinCode": "ABC123" }
    ```
  ...
  - **Possible Errors**:
    ...
    - `404 Company not found for the given code` – If the join code does not match any company.
    - `400 Invalid join code` – If the join code is malformed or missing.
  - **Flow**:
    1. The user must be authenticated (token/session required).
    2. The system validates the join code and fetches the corresponding company.
    3. If valid, the user is assigned to the company and the updated user resource is returned.
    4. If invalid or not found, an error is returned.
...
- `POST /api/v1/groups` - Create a new chat group.
  - **Input Example**: `{ "name": "General Chat", "description": "All hands", "imageUrl": "url", "visibility": "PUBLIC", "memberIds": ["uuid"], "createdBy": "uuid" }`
